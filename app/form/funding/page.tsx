        ...
        <div className="w-full">
            <div className="lg:w-[50vw] md:mx-auto sm:px-4 md:px-12 py-12 flex flex-col flex-nowrap bg-gray-100 text-black">
                ...
                <div className="w-full flex flex-col flex-nowrap mx-auto my-2 fade-in text-black">
                    ...
                    <textarea name={form?.section4.question1.key} value={form?.section4.question1.answer} onChange={e => updateAnswer(form?.section4.question1.key, e.target.value)} rows={2} placeholder="e.g. I am trying to raise ..." className="mx-4 my-2 px-3 py-4 text-lg rounded-md border border-gray-400" />
                    <label htmlFor={form?.section4.question2.key} className="mx-4 px-2 pt-4 font-bold">{form?.section4.question2.question}</label>
                    <textarea name={form?.section4.question2.key} value={form?.section4.question2.answer} onChange={e => updateAnswer(form?.section4.question2.key, e.target.value)} rows={2} placeholder="e.g. The long term goals ..." className="mx-4 my-2 px-3 py-4 text-lg rounded-md border border-gray-400" />
                    <label htmlFor={form?.section4.question3.key} className="mx-4 px-2 pt-4 font-bold">{form?.section4.question3.question}</label>
                    <textarea name={form?.section4.question3.key} value={form?.section4.question3.answer} onChange={e => updateAnswer(form?.section4.question3.key, e.target.value)} rows={2} placeholder="e.g. The exit strategy ..." className="mx-4 my-2 px-3 py-4 text-lg rounded-md border border-gray-400" />
                    <label htmlFor={form?.section4.question4.key} className="mx-4 px-2 pt-4 font-bold">{form?.section4.question4.question}</label>
                    <textarea name={form?.section4.question4.key} value={form?.section4.question4.answer} onChange={e => updateAnswer(form?.section4.question4.key, e.target.value)} rows={2} placeholder="e.g. One of my biggest concerns ..." className="mx-4 my-2 px-3 py-4 text-lg rounded-md border border-gray-400" />
                    <label htmlFor={form?.section4.question5.key} className="mx-4 px-2 pt-4 font-bold">{form?.section4.question5.question}</label>
                    <textarea name={form?.section4.question5.key} value={form?.section4.question5.answer} onChange={e => updateAnswer(form?.section4.question5.key, e.target.value)} rows={2} placeholder="e.g. The timeline to achieve ..." className="mx-4 my-2 px-3 py-4 text-lg rounded-md border border-gray-400" />
                </div>
                ...
                <div className="w-full flex flex-row flex-nowrap mx-auto mt-4 mb-2 justify-between items-center fade-in">
                    ...
                    <div className="flex flex-row flex-nowrap justify-end items-center">
                        <button type="button" onClick={saveDraft} className="mr-4 py-1 px-3 rounded-md text-lg tracking-wider bg-indigo-700 text-white hover:bg-indigo-800">Save Draft</button>
